...
void gameOver() {
    ...
    // dimensiones de la textura game over
    int texW, texH;
    SDL_QueryTexture(gameOverTexture, NULL, NULL, &texW, &texH); // obtener las dimensiones de la textura

    // coordenadas para centrar la textura de game over en la pantalla
    int gameOverWidth = texW;  //ancho de la textura cargada
    int gameOverHeight = texH; // altura de la textura cargada
    int gameOverX = (SCREEN_WIDTH - gameOverWidth) / 2; // centrar horizontalmente
    int gameOverY = (SCREEN_HEIGHT - gameOverHeight) / 2; // centrar verticalmente

    render(gameOverTexture, gameOverX, gameOverY, gameOverWidth, gameOverHeight); // dibujar la textura de game over
    SDL_DestroyTexture(gameOverTexture); // liberar la textura de game over
}
